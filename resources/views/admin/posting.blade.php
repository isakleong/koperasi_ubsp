...
@section('content')
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="py-3 mb-4">
            <span class="fw-light">Posting Jurnal</span>
        </h4>
        <div class="row">
            <div class="col-xl">
                <div class="card mb-4">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h5 class="mb-0">Posting Jurnal</h5>
                                ...
                                <tbody>
                                    ...
                                    @foreach ($transaction as $item)
                                        <tr>
                                            ...
                                            <td>
                                                @foreach($item->debitDetail as $index => $detail)
                                                    @if($index > 0)
                                                        <hr style="border-top: 1.5px dashed #bbb;">
                                                    @endif
                                                    {{ $detail->account->accountNo }} - {{ $detail->account->name }}
                                                    <br>
                                                    Rp {{ number_format($detail->total, 2, '.', ',') }}
                                                @endforeach
                                            </td>
                                            <td>
                                                @foreach($item->creditDetail as $index => $detail)
                                                    @if($index > 0)
                                                        <hr style="border-top: 1.5px dashed #bbb;">
                                                    @endif
                                                    {{ $detail->account->accountNo }} - {{ $detail->account->name }}
                                            ...
                                            <td>Rp {{ number_format($item->totalDebit, 2, '.', ',') }}</td>
                                            <td>{{ $item->notes }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
